...
import binascii
from mobile_insight_enb.mobile_insight.monitor.dm_collector.dm_endec.ws_dissector import WSDissector
  ...
  def handle_pdcp_dl(self):
    ...
    try:
      if self.content is None:
        ...
        if packet[-1] == '\n':
            packet = packet[0:-1]
        l = re.split(", |: ", " ".join(packet.split(" ")[2:]))
        
        for i in range(0, len(l)-1, 2):
          d[l[i]] = l[i+1] if l[i] == 'timestamp' else int(l[i+1])
  ...
  def handle_mac_ul(self):
    ...
    try: 
      if self.content is None:
        ...
        # todo: there are other formats of this type of message have not been handled yet
        d['Frame Num'] = int(l[4])
        d['Subframe Num'] = int(l[6])
        d['type'] = l[2]
        for i in range(7, len(l)-1, 2):
          d[l[i]] = int(l[i+1])
        self.content = d
    finally:  
      return self.content
  ...
  def handle_rlc_ul(self):
    ...
    try:
      if self.content is None:
        ...
        # todo: cannot interpret the meaning of the numbers in the message
        d = {}
